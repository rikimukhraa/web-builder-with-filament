...
@endphp
<li class=" group">
    <a @if (!$place || $rute == 'home') href="{{ url($rute) }}" @endif
    ...
    @if ($child->children->isNotEmpty() && $place)
        <div
            class="dropdown-content absolute left-0 right-0 justify-center mx-auto mt-6 w-full  max-w-screen-md  bg-white/60 dark:bg-green-900/40 backdrop-blur-md shadow-lg rounded-b-xl border border-t-0 border-green-700/50">
            <div class="grid px-4 py-6 mx-auto text-gray-900 dark:text-white md:{{ $class }} md:px-6">
                @forelse ($child->children as $c)
                    <ul>
                        <li>
                            <a href="{{ $c->slug }}"
                                class="block p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-green-700/20">
